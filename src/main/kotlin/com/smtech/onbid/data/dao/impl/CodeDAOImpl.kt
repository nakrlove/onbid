...
class CodeDAOImpl(@Autowired val attachCodeRepository: CodeRepository): CodeDAO {
    ...
    override fun findListsEntity(attach: Codes, page: PageRequest): Page<Codes> {
        ...
        return if (attach.name.isNullOrBlank()) {
            ...
            val resultData:Page<Codes> = attachCodeRepository.findByNameContaining(attach.name!!,page)
//            val count = attachCodeRepository.findNameCountQuery(attach.name!!)
//            PageImpl(resultData,page,count)
            resultData
        }
